...
exports.handleWebhook = async (req, res) => {
  ...
  try {
    ...
    for (const session of allSessions) {
      ...
      // ✅ CASE 3: Question response handling
...
async function processAINodeSimple(currentNode, receiver, apiUrl, accessToken, flowId, sender, edges, sendername, username) {
  ...
  try {
    ...
    for (const route of smartRoutes) {
      ...
      const confidence = route.keywords.length > 0 ? score / route.keywords.length : 0;
    ...
    if (!matchedRoute) {
      logger.info("No route matched, sending fallback");
      ...
      await logToPM2({
        ...
        senderName: sendername,
        logType: 'ai_fallback',
        message: 'No route matched, sending fallback',
        userMessage,
...
async function storeUserResponse(sender, receiver, flowId, variableName, variableValue, username) {
  try {
    let storedValue = variableValue;
    if (!isNaN(variableValue)) {
      const buttonText = await getButtonTextById(variableValue, flowId);
      if (buttonText) {
    ...
    await logToPM2({
      ...
      logLevel: 'info',
      message: `User response stored: ${variableName} = ${storedValue}`,
    ...
    logger.info(`✅ User response successfully saved: ${storedValue}`);
  } catch (error) {
...
async function replaceVariables(text, sender, receiver, flowId, extra = {}, sendername = null) {
